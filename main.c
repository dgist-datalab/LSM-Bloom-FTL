...
int main(){
	...
	for(uint32_t j=1; j<=16; j*=2){
		...
		/*
		printf("\n");
		for(uint32_t i=TESTLEVELSTART; i<=TESTLEVELSTART+3; i++){
			size_factor=get_size_factor(i, BLOCKNUMBER/j);
			printf("HYB\t%d\t%d\t%d\t%d\t%.2f\t%ubits\n",i,size_factor,get_waf(HYBRID,i , size_factor), 
					get_raf(HYBRID, i, size_factor),
					(float)(get_sparse_sorted_head(i,size_factor)*j)/BLOCKNUMBER,
					get_number_of_bits_for_block_fpr(BLOOMFPR/get_raf(HYBRID,i ,size_factor)));
		}*/
		printf("=================================================\n");
	}
